...
const App = () => {
  ...
  const filterProductsPage = (filters) => {
    ...
    const filteredByFilters = productsItems.filter((product) => {
      const colorMatch = filterMatch(color, product.color);
      const typeMatch = filterMatch(type, product.type);
      const genderMatch = filterMatch(gender, product.gender);
  ...
  /**
   * @param {object} item - The item incremented.
   */
  const handleCartIncrement = (item) => {
    setCountQty((prevItemQty) => {
      const newQty = prevItemQty[item.id] + 1;
      if (newQty <= item.quantity) {
        return { ...prevItemQty, [item.id]: newQty };
      }
      return prevItemQty;
    });
  };
  ...
  const handleCartDelete = (item) => {
    ...
    const updatedQuantites = { ...countQty };
    delete updatedQuantites[item.id];
    setCountQty(updatedQuantites);
